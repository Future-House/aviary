...
try:
    EXTRA_FASTA_URL = os.environ["AVIARY_CLONING_EXTRA_FASTA_URL"]
except KeyError:
    raise ValueError(
        "AVIARY_CLONING_EXTRA_FASTA_URL is not set. "
        "Please provide a URL to a downloadable FASTA file with "
        "annotations (e.g. Kozak sequence). "
    ) from None
try:
    FPBASE_FASTA_URL = os.environ["AVIARY_CLONING_FPBASE_FASTA_URL"]
except KeyError:
    raise ValueError(
        "AVIARY_CLONING_FPBASE_FASTA_URL is not set. "
        "Please provide a URL to a dump of the fluorescent protein "
        "database. "
    ) from None
...
image = (
    ...
    .run_commands(f"wget {EXTRA_FASTA_URL} -O extra.fasta")
    .run_commands(f"wget {FPBASE_FASTA_URL} -O fpbase.fasta")
...
def annotate(fasta_str: str, circular: bool) -> str:
    ...
    with tempfile.NamedTemporaryFile("w", encoding="utf-8") as f:
        ...
        from annotation2gbk import annotation2genbank  # type: ignore[import-not-found]
    ...
    with open(f"{name}.gbk", encoding="utf-8") as f:  # noqa: FURB101
        return f.read()
