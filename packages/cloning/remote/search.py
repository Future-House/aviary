...
with image.imports():
    ...
    def get_schema():
        ...
        schema_builder.add_text_field("docid", stored=True)
...
def build_index():
    index = get_index()
    records: dict[str, list[SearchResult]] = {}
    writer = index.writer()
    ...
    count = 0
    batch_size = 2**10
    files = list(Path("/seqs").rglob("*.gbk"))
...
class PlasmidSearch:
    ...
    def setup(self):
        ...
        records_path = Path("/search/records.pkl")
        if records_path.exists():
            with records_path.open("rb") as f:
                self.records: dict[str, list[SearchResult]] | None = pickle.load(f)  # noqa: S301
        else:
            logger.warning("No records found, search is not supported")
            self.records = None
        self.index.reload()
    ...
    async def search(self, query: str, limit: int = 10) -> list[SearchResult]:
        ...
        assert self.records is not None, (
            "Records were not found in index volume, cannot perform search."
        )
