...
class HotPotQADataset(TaskDataset[HotPotQAEnv]):
    ...
    def get_data_from_hugging_face(
        ...
        return [
            (
                # Right zero-pad the ID to make a UUID4. Unfortunately left padding can
                # lead to collisions when converting to integers via `UUID(...).int`
                UUID(bytes=bytes.fromhex(d["id"]) + b"\x00" * 4, version=4),
                d["question"],
                d["answer"],
            )
            for d in data
